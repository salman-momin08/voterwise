...
const ChatAssistant: React.FC<ChatAssistantProps> = ({ context, onClose }) => {
  ...
  const handleSend = async (text: string) => {
    ...
    const assistantMsg: Message = {
      ...
      is_validated: responseText.includes('Source:'),
      actions: [] 
    };
        ...
        {messages.length === 0 && (
          <div className="chat-welcome">
            ...
            <div className="quick-prompts">
              <button onClick={() => handleSend("How do I register to vote?")} className="prompt-chip">
                How to register?
              </button>
              <button onClick={() => handleSend("When is the next election deadline?")} className="prompt-chip">
                Next deadline?
              </button>
              <button onClick={() => handleSend("What ID do I need to vote?")} className="prompt-chip">
                Required ID docs
              </button>
            </div>
          </div>
